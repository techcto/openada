...
import { ArrowLeft, ExternalLink, FileSearch, Globe2, LoaderCircle, ScanSearch, ShieldCheck } from 'lucide-react'
          ...
          <div className="directory-page">
            <div className="directory-heading">
              <div>
                ...
                <h1>{page ? page.title || new URL(page.sourceUrl).pathname : scan ? `Scan from ${formatDate(scan.completedAt || scan.createdAt)}` : site ? site.hostname : 'The public scan directory'}</h1>
            ...
            {loading ? <section className="loading"><LoaderCircle className="spin" size={32} aria-hidden /><strong>Loading the public archive</strong></section> : page ? <PageView page={page} /> : scan ? <ScanView site={site} scan={scan} pages={data.pages || []} /> : site ? <SiteView site={site} scans={data.scans || []} /> : <SiteList sites={data.sites || []} />}
          </div>
        </main>
      </OpenAdaShell>
      <style jsx>{`
        ...
        .site-grid, .scan-grid { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 14px; }
        .site-card, .scan-card { min-height: 214px; display: flex; flex-direction: column; border: 1px solid #dce3ea; border-radius: 8px; background: #fff; padding: 18px; color: #172033; text-decoration: none; box-shadow: 0 12px 26px rgba(23, 32, 51, .06); }
        .site-card:hover, .site-card:focus-visible, .scan-card:hover, .scan-card:focus-visible, .page-row:hover, .page-row:focus-visible { border-color: #25635f; transform: translateY(-2px); }
        ...
        .card-top { color: #25635f; font-size: .78rem; font-weight: 800; } .site-card h2, .scan-card h2 { margin-top: 30px; } .site-card p, .scan-card p { margin-top: 5px; color: #64748b; font-size: .9rem; overflow-wrap: anywhere; }
        ...
        @media (max-width: 820px) { .directory-heading { display: grid; align-items: start; } .site-grid, .scan-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); } }
        @media (max-width: 620px) { .directory-page { padding: 18px; } .site-grid, .scan-grid, .detail-metrics { grid-template-columns: 1fr; } .detail-metrics { display: grid; } .page-row { align-items: flex-start; padding: 14px 0; } .page-row > span:last-child { text-align: right; } }
      `}</style>
    </>
  )
}
...
function SiteList({ sites }: { sites: Site[] }) {
  return <section className="site-grid" aria-label="Scanned sites">
    {sites.length === 0 ? <div className="empty"><ShieldCheck size={24} aria-hidden /><h2>Be the first site in the directory</h2><p>Submit a public site scan and its history will appear here.</p></div> : sites.map((site) => <a className="site-card" href={`/directory/${encodeURIComponent(site.id)}`} key={site.id}>
      <div className="card-top"><Globe2 size={19} aria-hidden /><span>{formatDate(site.lastScanAt)}</span></div><h2>{site.displayName}</h2><p>{site.hostname}</p><div className="card-meta"><strong>{site.latestGrade ?? '--'}</strong><span>{site.latestScore ?? '--'} / 100</span><span>{site.pageCount} page{site.pageCount === 1 ? '' : 's'}</span></div>
    </a>)}
  </section>
}
...
function SiteView({ site, scans }: { site: Site; scans: Scan[] }) {
  return <section aria-labelledby="scan-history-heading">
    ...
    <div className="scan-grid">{scans.length === 0 ? <div className="empty"><FileSearch size={24} aria-hidden /><h2>No scans recorded yet</h2><p>This site is in the directory, but no completed archive record is available.</p></div> : scans.map((scan) => <a className="scan-card" href={`/directory/${encodeURIComponent(site.id)}/scans/${encodeURIComponent(scan.id)}`} key={scan.id}>
      <div className="card-top"><ScanSearch size={19} aria-hidden /><span>{formatDate(scan.completedAt || scan.createdAt, true)}</span></div><h2>{scan.status === 'completed' ? 'Accessibility scan' : `Scan ${scan.status}`}</h2><p>{scan.url || site.hostname}</p><div className="card-meta"><strong>{scan.grade ?? '--'}</strong><span>{scan.score ?? '--'} / 100</span><span>{scan.pagesScanned} page{scan.pagesScanned === 1 ? '' : 's'}</span></div>
    </a>)}</div>
  </section>
}
...
function PageView({ page }: { page: PageDetail }) {
  ...
  return <section className="page-detail" aria-labelledby="page-detail-heading">
    ...
    <div className="detail-metrics"><div className="metric"><span>ADA grade</span><strong className={page.ada && page.ada.score >= 85 ? 'grade-good' : 'grade-review'}>{page.ada?.grade || '--'}</strong></div><div className="metric"><span>ADA score</span><strong>{page.ada?.score ?? '--'} / 100</strong></div><div className="metric"><span>WCAG issues</span><strong className={violations.length === 0 ? 'grade-good' : 'grade-review'}>{page.ada?.violationsCount ?? violations.length}</strong></div><div className="metric"><span>Language issues</span><strong className={page.languageErrors === 0 ? 'grade-good' : 'grade-review'}>{page.languageErrors}</strong></div></div>
